...
For this experiment the WTA network will be shown images of either horizontal or vertical bars. Through unsupervised learning the output neurons shall learn to cluster the images together, depending on the orientation and position of the bars. 
...
As every pixel of the shown images was connected to two input neurons the network had 1682 input neurons. Each of these neurons had a firing frequency of 20 Hz, when in the active state. Then there were ten output neurons, one for each possible group. There were two different groups of prior neurons, one of them being active when the image is of horizontal orientation and the other one being active for vertical orientation. The number of prior neurons and their firing frequency had to be determined via grid search, as they needed to be set at values where the impact of the a-priori information is neither to strong nor to weak.

\paragraph{Network parameters}

%TODO whole subsection
dt = 0.001 # seconds  (chosen by me)
sigma = 0.01 (acc to nessler)

c = 20   .... already determined in rotated bars...
lambda = 10^-3  ... already determined in rotated bars...
Before doing this experiment as a proof of concept Experiment 2 of \citet{nessler} was reproduced. Within this experiment the parameters $c = 20$ and $\lambda = 10^{-3}$ were determined via grid search.

The time constant for the rise of the signal $\tau_{rise}$ was 1 ms and the time constant for the decay of the signal $\tau_{decay}$ was 15 ms. (acc to nessler)
tauRise = 0.001
tauDecay = 0.015
