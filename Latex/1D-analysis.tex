...
After calculating all entries of $P^{X|Y}$, according to the first term of Equation \ref{eqn:P1}, its rows were element-wise taken to the power the input image vector
\begin{equation}
\label{eqn:p1XvorausgesetztYMalX}
P_1(X = \underline{x}|Y = k) = (P^{X|Y}_{k,*})^{\underline{x}} 
\end{equation}
resulting in a conditional probability that the input vector of the active pixels belongs to output class $k$. The input vector was given with entries of 1 for white pixels and with entries of 0 for black pixels.
Next, to include the contribution of the input neurons that are spiking when a pixel is black, the conditional probability of the input neurons that are active for the entries, where $x_i = 0$, had to be calculated. To achieve this complementary conditional probability at first $P^{X|Y}$ was subtracted from one. To include the correct conditional probabilities for the complementary case the input image vector x was then also subtracted from one. $1 - P^{X|Y}$ was element-wise taken to the power of $1 - \underline{x}$ to yield
\begin{equation}
...
According to Equation \ref{eqn:P2} $P(Y=k|Z=j)$ was then obtained by
\begin{equation}
\label{eqn:pYvorausgesetztZ}
P(Y=k|Z=j) = (P^{Y|Z})^z,
\end{equation}
where z is given by a 4 x 1 one-hot encoded vector of the prior.
Finally, inserting $P(X = \underline{x}|Y = k)$ and $P(Y=k|Z)$ into Equation \ref{eqn:pYvorausgesetztXUndZ} yields the analytical posterior $P_{analysis}(Y = k|X = \underline{x}, Z)$.
