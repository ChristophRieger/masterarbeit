...
After calculating all entries of $P^{X|Y}$, according to the first term of Equation \ref{eqn:P1}, the conditional probability that the encoded input vector of the white pixels belongs to output class $k$ is given by
\begin{equation}
\label{eqn:p1XvorausgesetztYMalX}
P_1(X = \underline{x}|Y = k) = \prod_{i = 1}^N(P^{X|Y}_{k,i})^{x_i},
\end{equation}
where $x_i$ an encoded pixel value.
Next, to include the contribution of the input neurons that are spiking when a pixel is black, the conditional probability of the input neurons that are active for $x_i = 0$, had to be calculated. To obtain this complementary conditional probability $P_2(X = \underline{x}|Y = k)$ at first $P^{X|Y}$ was subtracted from one. To consider black instead of white pixels for the complementary case $\underline{x}$ was then also subtracted from one. This yielded
\begin{equation}
\label{eqn:p2minusXvorausgesetztYMalX}
P_2(X = \underline{x}|Y = k) = \prod_{i = 1}^N(1 - P^{X|Y}_{k,i})^{(1 - x_i)}.
\end{equation}
...
\end{equation}
where z is given by a 4 x 1 one-hot encoded vector of the prior and $z_j$ is one of its values.
...
The input weights for the simulation were calculated as two separate sets. First, weights $w^{I}$ for the input neurons that are active for active input pixels were determined by 
\begin{equation}
\label{eqn:1DWeights}
w^{I} = \ln(P^{X|Y}).
\end{equation}
Then, complementary input weights $\overline{w}^{I}$ were calculated for  input neurons representing non active input pixels with
\begin{equation}
\label{eqn:1DWeightsComplementary}
\overline{w}^{I} = \ln(1 - P^{X|Y}).
\end{equation} 
...
\subparagraph{$\boldsymbol{\tau_{decay}} \text{ = 0.015 seconds, }\mathbf{f_{prior}} \text{ = 0 Hz}$}
...
\subparagraph{$\boldsymbol{\tau_{decay}} \text{ = 0.004 seconds, }\mathbf{f_{prior}} \text{ = 0 Hz}$}
...
\end{table}
\subparagraph{$\boldsymbol{\tau_{decay}} \text{ = 0.004 seconds, }\mathbf{f_{input}} \text{ = 88 Hz}$}
...
\end{table}
\subparagraph{$\boldsymbol{\tau_{decay}} \text{ = 0.004 seconds, }\mathbf{f_{input}} \text{ = 98 Hz, }\mathbf{f_{prior}} \text{ = 440 Hz}$}
