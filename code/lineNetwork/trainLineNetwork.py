...
for t in np.arange(0, simulationTime, dt):
  ...
  if prior == 0:
    for priorIterator in range (0, int(numberZNeurons/2)):
      if poissonGenerator.doesNeuronFire(AfiringRate, dt):
        ZSpikes[0].append(t)
        ZSpikes[1].append(priorIterator)
  elif prior == 1:
    for priorIterator in range (int(numberZNeurons/2), numberZNeurons):
      if poissonGenerator.doesNeuronFire(AfiringRate, dt):
        ZSpikes[0].append(t)
        ZSpikes[1].append(priorIterator)
  ...
  expiredASpikeIDs = []
  for i, ANeuron in enumerate(ZSpikes[1]):
    ...
    if ZSpikes[0][i] < t - sigma:
      ...
      ATilde[ZSpikes[1][i]] = ATildeFactor * kernel.tilde(t, dt, ZSpikes[0][i], tauRise, tauDecay)
      for k in range(numberYNeurons):
        U[k] += priorWeights[ANeuron, k] * ATilde[ZSpikes[1][i]]
...
directoryPath =  "c" + str(c) + "_eta" + str(learningRateFactor) + "_ATildeFactor" + str(ATildeFactor) + "_numberPriorNeurons" + str(numberZNeurons)
if not os.path.exists(directoryPath):
  os.mkdir(directoryPath)
np.save(directoryPath + "/c" + str(c) + "_eta" + str(learningRateFactor) + "_ATildeFactor" + str(ATildeFactor) + "_YZWeights"  + "_numberPriorNeurons" + str(numberZNeurons) + ".npy", weights)
np.save(directoryPath + "/c" + str(c) + "_eta" + str(learningRateFactor) + "_ATildeFactor" + str(ATildeFactor) + "_AZWeights"  + "_numberPriorNeurons" + str(numberZNeurons) + ".npy", priorWeights)
